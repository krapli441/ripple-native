...
function MakeRippleScreen(): React.ReactElement {
  ...
  useEffect(() => {
    const fetchRandomTags = async () => {
      try {
        const response = await fetch('http://3.34.138.35:3000/tags/random');
        if (!response.ok) {
  ...
  useEffect(() => {
    if (route.params?.selectedTags) {
      const updateTagsWithSelected = async () => {
        try {
          const response = await fetch('http://3.34.138.35:3000/tags/all');
          if (!response.ok) {
            throw new Error('Server error');
          }
  ...
  const createRipple = async (rippleData: any) => {
    try {
      const response = await fetch('http://3.34.138.35:3000/ripples', {
        method: 'POST',
        headers: {
